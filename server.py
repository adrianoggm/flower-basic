...
class MQTTFedAvg(fl.server.strategy.FedAvg):
    ...
    def __init__(self, model: ECGModel, mqtt_client: mqtt.Client, **kwargs):
        """
    ...
    def aggregate_fit(
        ...
        try:
            ...
            if isinstance(new_parameters, tuple):
                ...
                print(f"[SERVER] DEBUG: Extraída tupla, tipo interno: {type(parameters_obj)}")
            else:
            ...
            if hasattr(parameters_obj, 'tensors'):
                ...
                param_arrays = [fl.common.bytes_to_ndarray(tensor) for tensor in param_arrays]
                print(f"[SERVER] DEBUG: Usando parameters_obj.tensors")
            else:
                # Flower anterior - usar la función de conversión
                param_arrays = fl.common.parameters_to_ndarrays(parameters_obj)
                print(f"[SERVER] DEBUG: Usando parameters_to_ndarrays")
            ...
            if self.mqtt is not None:
                ...
                print(f"[SERVER] ✅ Modelo global publicado en MQTT topic: {MODEL_TOPIC}")
            else:
                print("[SERVER] MQTT no disponible, saltando publicación")
                
            print(f"[SERVER] Modelo global agregado exitosamente")
            
        except Exception as e:
            print(f"[SERVER] ERROR en publicación MQTT: {e}")
            import traceback
            traceback.print_exc()
            print(f"[SERVER] Continuando sin publicación MQTT")
