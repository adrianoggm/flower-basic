...
def swell_analysis_real_participants():
    ...
    print(f"  🎯 ACCURACY SINTÉTICO: {accuracy_synthetic:.3f} ({accuracy_synthetic*100:.1f}%)")
    print()
    
    # === COMPARACIÓN Y DIAGNÓSTICO ===
    print("📊 COMPARACIÓN FINAL")
    print("-" * 20)
    print(f"🧑‍🔬 Participantes REALES (25):    {accuracy_real:.3f} ({accuracy_real*100:.1f}%)")
    print(f"🤖 Participantes SINTÉTICOS (500): {accuracy_synthetic:.3f} ({accuracy_synthetic*100:.1f}%)")
    print()
    
    if accuracy_synthetic > accuracy_real + 0.05:
        print("🚨 DIAGNÓSTICO: Los participantes sintéticos inflan artificialmente el accuracy")
        print("   Razones:")
