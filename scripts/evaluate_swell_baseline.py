...
class SWELLBaselineEvaluator:
    ...
    # Samples are ONLY for tests (test_*.py files)
        ...
        for modality, patterns in modality_patterns.items():
            ...
            for pattern in patterns:
                ...
                try:
                    ...
                    print(f"  ✓ Loaded REAL {modality} data: {df.shape} from {file_path.name}")
                    ...
                    feature_cols = [col for col in df.columns 
                                  if col not in ['participant', 'subject', 'condition', 'time', 'timestamp', 'pp', 'blok', 'c']]
                    df = df.rename(columns={col: f"{modality}_{col}" for col in feature_cols})
        ...
        for i, df in enumerate(dataframes[1:], 1):
            ...
            merged_block_cols = [col for col in merged_df.columns if any(block in col for block in ['blok', 'block', '_c'])]
            df_block_cols = [col for col in df.columns if any(block in col for block in ['blok', 'block', '_c'])]
            ...
            if merge_cols:
                ...
                merged_df = pd.merge(merged_df, df, on=merge_cols, how='inner', suffixes=('', f'_{i}'))
                ...
                if merged_df.shape[0] > 50000:
                    print(f"      Dataset too large ({merged_df.shape[0]} rows), sampling 50k rows...")
                    merged_df = merged_df.sample(n=50000, random_state=42)
            else:
                print(f"      Warning: No common merge columns found, skipping this modality")
                continue
        ...
        subject_col = None
        for possible_col in ['participant', 'subject', 'participantno', 'participant_id', 'id']:
            if possible_col in merged_df.columns:
                subject_col = possible_col
                break
        
        if subject_col is None:
            print("  No subject column found in real data, creating synthetic subject IDs")
            # Create subject IDs based on data rows (realistic for merged data)
            merged_df['participant'] = [f"P{(i//100) + 1:02d}" for i in range(len(merged_df))]
            subject_col = 'participant'
        
        # Get feature columns (exclude subject, condition, and non-numeric columns)
        non_feature_cols = [subject_col, 'condition', 'timestamp', 'timestamp_1', 'timestamp_2', 'timestamp_3', 
                           'blok', 'pp', 'c']
        ...
        condition_mapping = {
            ...
            'N': 0, 'n': 0,  # Normal condition (no stress)
            'T': 1, 't': 1,  # Time pressure (stress)
            'I': 1, 'i': 1,  # Interruptions (stress)
            'R': 1, 'r': 1,  # Combined stress conditions
            
            # Alternative mappings for other possible formats
            'normal': 0, 'baseline': 0, 'control': 0, 'no stress': 0,
            'time pressure': 1, 'interruption': 1, 'interruptions': 1, 
            'combined': 1, 'stress': 1
        }
        ...
        for condition in conditions:
            ...
            elif any(stress_word in condition_lower for stress_word in ['stress', 'pressure', 'interrupt', 'high', 'load']):
                y.append(1)  # Stress
            elif any(baseline_word in condition_lower for baseline_word in ['baseline', 'control', 'normal', 'rest', 'low']):
        ...
        if unmapped_conditions:
            print(f"  ⚠️  Unmapped conditions (defaulted to no-stress): {unmapped_conditions}")
    ...
    def train_multimodal_network(
        ...
        class MultimodalSWELLNet(nn.Module):
            ...
            def forward(self, x):
                ...
                # Fuse modalities
                fused = torch.cat([computer_out, facial_out, posture_out, physiology_out], dim=1)
    ...
    def run_evaluation(self) -> Dict:
        ...
        # Train multimodal neural network
        nn_results = self.train_multimodal_network(X_train, X_val, X_test, y_train, y_val, y_test)
