...
class ECG5000DeprecationChecker:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                for violation_type, pattern_info in self.deprecated_patterns.items():
                    ...
                    if re.search(pattern, line_clean, re.IGNORECASE):
                        ...
                        violations.append(DeprecationViolation(
                            file_path=file_path,
    ...
    def _is_legitimate_usage(self, line: str, file_path: Path) -> bool:
        ...
        if 'test_' in file_path.name and ('deprecated' in line.lower() or 'legacy' in line.lower()):
            return True
        
        # Allow in migration scripts or documentation
        if any(term in str(file_path).lower() for term in ['migration', 'deprecated', 'legacy', 'docs']):
            return True
        
        # Allow if line contains migration context
        if any(term in line.lower() for term in ['deprecated', 'migrate', 'old', 'legacy', 'replace']):
            return True
