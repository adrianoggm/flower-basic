...
def validate_real_data_usage(dataset_name: str, data_path: str) -> bool:
    ...
    if dataset_name not in APPROVED_DATASETS:
        raise ValueError(f"Dataset '{dataset_name}' not in approved list: {list(APPROVED_DATASETS.keys())}")
...
def check_mock_data_prohibition():
    ...
    prohibited_functions = [
        'generate_mock_', 'create_fake_', 'simulate_', 'synthetic_',
        'random_data', 'mock_data', 'fake_data', 'artificial_'
    ]
        ...
        if inspect.isfunction(obj):
            for prohibited in prohibited_functions:
                if prohibited in name.lower():
                    raise RuntimeError(f"PROHIBITED: Mock data function '{name}' detected! Remove immediately.")
