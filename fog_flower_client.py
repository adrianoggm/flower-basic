...
class FogClient(fl.client.NumPyClient):
    ...
    def __init__(self, server_address: str):
        ...
        self.mqtt.on_connect = lambda c, u, f, rc, p=None: c.subscribe(PARTIAL_TOPIC)
        self.mqtt.on_message = self._on_partial
        self.mqtt.connect(MQTT_BROKER, MQTT_PORT)
        self.mqtt.loop_start()
        print(f"[FOG_CLIENT] Conectado a MQTT broker, escuchando agregados parciales en {PARTIAL_TOPIC}")
    ...
    def fit(self, parameters: List[np.ndarray], config):
        ...
        timeout_count = 0
        while self.partial_weights is None and timeout_count < 60:  # 30 segundos timeout
            time.sleep(0.5)
            timeout_count += 1
        ...
        print(f"[FOG_CLIENT] Enviando agregado parcial al servidor central ({num_samples} muestras)")
...
def main():
    ...
    fl.client.start_numpy_client(
        ...
        client=FogClient("localhost:8080")
    )
